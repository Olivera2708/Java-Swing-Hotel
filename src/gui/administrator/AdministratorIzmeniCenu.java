...
public class AdministratorIzmeniCenu extends JFrame{
	...
	AdministratorIzmeniCenu (JTable tabela, boolean issoba, CenaUsluge usluga, CenaSobe tipSobe) {
		this.usluga = usluga;
	...
	private void prikaziNoviZaposleni() {
		...
		JLabel naslov = new JLabel("Izmeni cena");
		naslov.setFont(naslov.getFont().deriveFont(18f));
		panel.add(naslov, "center, span 2");
		
		if (!issoba){
			usluge.setSelectedItem(usluga.getUsluge().getTip());
			datumOd.setText(datum_formatter.format(usluga.getOdDatum()));
			datumDo.setText(datum_formatter.format(usluga.getDoDatum()));
			cena.setText(String.valueOf(usluga.getCena()));
		}
		else {
			usluge.setSelectedItem(tipSobe.getTipSobe().getTip());
			datumOd.setText(datum_formatter.format(tipSobe.getOdDatum()));
			datumDo.setText(datum_formatter.format(tipSobe.getDoDatum()));
			cena.setText(String.valueOf(tipSobe.getCena()));
		}
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (issoba) {
						for (CenaSobe s: manageAll.getCenovnikSobaManager().getAll()) {
							if (s.getTipSobe().getTip().equals(uslugaText) && (s.getId() != tipSobe.getId())){
								try {
						...
						for (CenaUsluge s: manageAll.getCenovnikUslugaManager().getAll()) {
							if (s.getUsluge().getTip().equals(uslugaText) && (s.getId() != usluga.getId())){
								try {
									if ((s.getOdDatum().before(datum_formatter.parse(datumDoText)) && s.getDoDatum().after(datum_formatter.parse(datumOdText)))
											|| (s.getDoDatum().before(datum_formatter.parse(datumDoText)) && s.getDoDatum().after(datum_formatter.parse(datumOdText)))
											|| s.getDoDatum().equals(datum_formatter.parse(datumOdText)) || s.getOdDatum().equals(datum_formatter.parse(datumDoText))) {
						...
						if (!datumOdText.matches(datumRegex) || !datumDoText.matches(datumRegex)){
							...
							try {
								if (datum_formatter.parse(datumOdText).after(datum_formatter.parse(datumDoText))) {
									...
									if (!issoba) {
										try {
											manageAll.getCenovnikUslugaManager().edit(usluga.getId(), manageAll.getUslugeManager().get_id(uslugaText), Integer.parseInt(cenaText), datum_formatter.parse(datumOdText), datum_formatter.parse(datumDoText));
										...
										try {
											manageAll.getCenovnikSobaManager().edit(tipSobe.getId(), manageAll.getTipSobeManager().get_id(uslugaText), Integer.parseInt(cenaText), datum_formatter.parse(datumOdText), datum_formatter.parse(datumDoText));
