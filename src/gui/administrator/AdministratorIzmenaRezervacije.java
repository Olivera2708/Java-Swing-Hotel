					...
					if (!datumOdText.matches(datumRegex) || !datumDoText.matches(datumRegex)){
						...
						try {
							if (datum_formatter.parse(datumOdText).after(datum_formatter.parse(datumDoText))) {
								...
								try {
									manageAll.getRezervacijeManager().edit(rezervacija.getId(), manageAll.getTipSobeManager().get_id(tipSobeText), lista_usluga, manageAll.getGostManager().get_id(korisnikText), datum_formatter.parse(datumOdText), datum_formatter.parse(datumDoText), statusText);
