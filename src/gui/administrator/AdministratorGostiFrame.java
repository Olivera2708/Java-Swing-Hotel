...
public class AdministratorGostiFrame extends JFrame{
	...
	private void allButtons() {
		btnAdd.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				AdministratorDodajGosta dodajGosta = new AdministratorDodajGosta(tabela);
				dodajGosta.setVisible(true);
			}
		...
		btnShow.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (gost == -1) {
					JOptionPane.showMessageDialog(null, "Morate selektovati gosta iz tabele.", "Greška", JOptionPane.WARNING_MESSAGE);
				}
				else {
					int id = (int) tabela.getValueAt(gost, 0);
					Gost izabran = manageAll.getGostManager().find(id);
					AdministratorPrikaziGosta prikaziGosta = new AdministratorPrikaziGosta(izabran);
					prikaziGosta.setVisible(true);
				}
			}
		});
		...
		btnEdit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (gost == -1) {
					...
					int id = (int) tabela.getValueAt(gost, 0);
					Gost izabran = manageAll.getGostManager().find(id);
					AdministratorIzmenaGosta izmenaGosta = new AdministratorIzmenaGosta(izabran);
					izmenaGosta.setVisible(true);
					osveziTabelu();
				}
			}
		});
	...
	private void areYouSure(int id) {
		...
		option[0] = "Da";
		option[1] = "Ne";
		int vrednost = JOptionPane.showOptionDialog(null, "Da li ste sigurni da želite da obrišete gosta?", "Izlazak", 0, JOptionPane.INFORMATION_MESSAGE, null, option, null);
		if (vrednost == JOptionPane.YES_OPTION) {
			manageAll.getGostManager().remove(id);
			osveziTabelu();
		}	
	}
